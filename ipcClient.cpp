...
int c_loadRequest_shm(char *msg)
{
  ...
  memset(mem_ServerConnect->text, '\0', LINE_MAX);
  strncpy(mem_ServerConnect->text, msg, (LINE_MAX-1));
...
int c_getResult_FIFO()
{
  ...
  if ((status = cs_makeFIFO(&fifo_fd, getpid(), fifoname)) == EXIT_SUCCESS)
	...
	{
	  num_in = 0;
	  memset(cbuf, '\0', LINE_MAX);

	  num_in = readLine(fifo_fd, cbuf, LINE_MAX);
